...
# Get raster and test files
raster_files <- list.files(path = "./5_NorthAmerica_prediction_outputs_250m_v92/2_RF/Prediction_outputs_tif/2_NA_mosaics", pattern = ".tif",
                           full.names = TRUE, recursive = TRUE)

test_files <- list.files(path = "./3_Training_and_Test_data_csv/3_northamerica_test_30pct", pattern = ".csv",
                         full.names = TRUE, recursive = TRUE)

validation_report <- matrix(data = NA, nrow = length(raster_files), ncol = 8)
validation_report <- as.data.frame(validation_report)
names(validation_report) <- c("Region", "Method", "Resolution", "Year",
...
for (i in 1:length(raster_files)) {
  ...
  # Extract year and biweek from file name
  year <- substr(basename(raster_files[i]), 36, 39)
  biweek <- substr(basename(raster_files[i]), 41, 42)

  # Put together a data frame for biweekly validation
  validation_data <- read.csv(test_files[i], header = TRUE, dec = ".")
  validation_data <- as.data.frame(cbind(validation_data$x, validation_data$y,
                                         validation_data$z))
  ...
  names(validation_data) <- c("X", "Y", "ESACCI_SM", "Year", "Biweek",
                              "Region", "Method", "Resolution")

  # Extract lat lon data from data
  validation_data <- as.data.frame(validation_data, xy = TRUE)
  xy_ <- validation_data[, c(1, 2)]

  # Convert coordinates into a spatial data frame
  ...
  # Extract raster values into the table
  ...
  final_validation_file <- as.data.frame(cbind(validation_data_table$X,
                                               ...
                                               validation_data_table$ESACCI_SM,
  ...
  # Add table labels
  names(final_validation_file) <- c("X", "Y", "Year", "Biweek", "ESACCI_ref",
                                    "SoilMoist_Pred", "Region", "Method", "Resolution")

  # Convert type for some table columns
  ...
  # Write report results for specific biweek
  csv_file <- paste0("./5_NorthAmerica_prediction_outputs_250m_v92/3_Validation_reports/1_Cross_validation/", year,
                     "/northamerica_rf_v92_250m_validation_test30pct_",
                     year, "_", biweek, ".csv")
  write.csv(final_validation_file, file = csv_file)

  # Add summary info to another table
  ...
  validation_report[i, 7] <- cor(final_validation_file$ESACCI_ref,
                                 final_validation_file$SoilMoist_Pred,
                                 use = "pairwise.complete.obs")
  validation_report[i, 8] <- rmse(final_validation_file$ESACCI_ref,
                                  final_validation_file$SoilMoist_Pred)
...
validation_file <- "./5_NorthAmerica_prediction_outputs_250m_v92/3_Validation_reports/1_Cross_validation/northamerica_rf_v92_250m_validation_test30pct.csv"
write.csv(validation_report, file = validation_file)
