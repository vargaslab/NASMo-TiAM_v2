...
# Load all DEM files with mean elevation
...
foreach(i = 1:length(DEM_files)) %dopar% {
  # Load the raster library within each parallel instance
  library(raster)

  # Get positional info from file name
  DEM_name <- sub("\\..*$", "", basename(DEM_files[i]))
  DEM_name <- substr(DEM_name, 1, 7)
  ...
  # Write reprojected results to GeoTIFF
...
# Merge together all reprojected DEMs into a stack
...
# Mosaic together all the DEM files to create a single image
...
# Write the mosaic results to a GeoTIFF
writeRaster(northamerica_mosaic, "./1_Preprocessed_data/2_NA_GMTED2010_terrain_parameters/1_GMTED2010_NorthAmerica_mosaic/NorthAmerica_GMTED2010_mosaic_LAEA_250m_temp.tif")
...
# This part of the code masks the preprocessed DEM mosaic to the North American boundary
...
# writeRaster(NA_DEM_final, "./1_Preprocessed_data/2_NA_GMTED2010_terrain_parameters/2_GMTED2010_NorthAmerica_mosaic_final/NorthAmerica_GMTED2010_mosaic_LAEA_250m.tif")
