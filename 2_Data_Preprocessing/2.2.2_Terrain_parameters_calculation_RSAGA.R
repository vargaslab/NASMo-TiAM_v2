...
# Create SAGA environment and specify the path with your SAGA executable installation
myenv <- rsaga.env(workspace=".", path="./saga-7.9.0_x64/", parallel=TRUE)

# Convert the DEM mosaic to the SAGA-compatible SGRD format
...
# Compute the desired terrain parameters
...
# Reprojection from LAEA CRS to WGS84 and transformation to TIF format.
# This process works in R, but it was performed in ArcPro as it preservers better the cell size when it goes from LAEA to WGS84.

# # Get all newly created SDAT raster terrain parameter files
...
#   # Import library for each parallel instance
#   library(raster)

#   # Reproject SDAT raster data to WGS84
#   rast_file <- raster(sdat_files[[i]])
#   rast_file <- projectRaster(rast_file, crs = WGS84_projection)

#   # Get name of the terrain parameter
#   file_name <- names(rast_file)

#   # Write reprojected results to file
